...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, Image, ImageBackground, Modal, Dimensions, Linking, Alert, BackHandler } from 'react-native'
...
const SearchDetailOther = (props) => {
    ...
    const getDetail = (word_id, word_name) => {
        setIsLoad(false)
        setWordName(word_name)
        setWordDetail([])
        ...
        if (id_language == '2') {
            fetch(APP_URL + 'view_efik_word', {
                ...
                .then((response) => response.json()).then((response) => {
                    global.global_loader_ref.show_loader(0);
                    console.log(JSON.stringify(response), "==view_efik_word");
                    if (response.status) {
                        if (response.data) {

                            setWordName(response.data[0].word_name.word)
                            word_id = response.data[0].word_id;
                            setWordId(response.data[0].word_id)
    ...
    const shareWord = async () => {
        const shareOptions = {
            ...
            title: word_name,
            url: "https://play.google.com/store/apps/details?id=" + PACKAGE_NAME, //images.image1, // places a base64 image here our your file path
            failOnCancel: false,
        };
        try {
        ...
        >
            ...
            <View style={{ flex: 1, backgroundColor: scheme === 'dark' ? 'rgba(0,0,0,0.8)' : 'rgba(0,0,0,0)', marginTop: scale(-20) }}>
                <View style={{ flexDirection: "row", alignSelf: "center", marginTop: scheme === 'dark' ? scale(-20) : scale(-5) }}>
                    ...
                    ) : <Spinner color={"black"} size={'small'} style={[styles.actionBase, { marginHorizontal: scale(15) }]} />}
                    <TouchableOpacity style={styles.actionBase}
                        onPress={() => {
                            shareWord()
                        }}
...
export default connect(mapStateToProps)(SearchDetailOther);
