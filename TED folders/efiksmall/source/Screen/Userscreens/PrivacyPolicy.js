...
const PrivacyPolicy = (props) => {
    ...
    const [cmsDataAboutPolicy, setCmsDataAboutPolicy] = useState('')
    useEffect(() => {
        getCms()
    ...
    const getCms = () => {
        ...
        fetch(APP_URL + 'get_cms_pages', {
            ...
            .then((response) => response.json()).then((response) => {
                ...
                if (response.status) {
                    if (response.data) {
                        setCmsDataAboutPolicy(response.data.privacy_policy.description)
                        global.global_loader_ref.show_loader(0);
                    }
                    else {
                        setCmsDataAboutPolicy(null)
                        global.global_loader_ref.show_loader(0);
                    }
                    global.global_loader_ref.show_loader(0);
                } else {
    ...
    return (
        ...
        >
            <View style={{ flex: 1, backgroundColor: scheme === 'dark' ? 'rgba(0,0,0,0.8)' : 'rgba(0,0,0,0)' }}>
                <Globalinclude.GlobalHeader name={'Privacy Policy'}
                    drawerIcon={false}
                    backIcon={true}
                    onPressBack={() => {
                ...
                <ScrollView>
                    <View style={{ paddingHorizontal: scale(20), paddingVertical: scale(20) }}>
                        <View style={{ paddingTop: scale(20) }}>
                            <HTMLView stylesheet={styles} value={cmsDataAboutPolicy}
